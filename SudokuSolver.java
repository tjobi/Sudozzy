...
public class SudokuSolver implements ISudokuSolver {
	...
	public void setValue(int col, int row, int value) {
		if (row > size*size || col > size*size || row < 0 || col < 0) {
			throw new RuntimeException(String.format("Row and col should be between 0 and %d. You said: 'row: %d', 'col: %d'", size*size, row, col));
		} else if (value < 0 || value > size*size) {
			throw new RuntimeException(String.format("Value should be between 0 and %d. You said: 'value: %d'", size*size, value));
		}
	...
	public void setup(int size1) {
		...
		for (int i = 0; i < puzzle.length; i++) {
			...
			for (int j = 0; j < row.length; j++) {
				int value = row[j];
				var dij = value == 0 ? Arrays.asList(1,2,3,4,5,6,7,8,9) : Arrays.asList(value);
				D.add(new ArrayList<Integer>(dij));
			}
		}
	}
	...
	public void readInPuzzle(int[][] p) {
		if (p.length != size*size) {
			throw new RuntimeException(String.format("Uh oh, that ain't supposed to be happening, puzzle-size: %d not the expected: %d", p.length, size*size));
		}
		for (int index = 0; index < p.length; index++) {
			int[] row = p[index];
			if (row.length != size*size) {
				throw new RuntimeException(String.format("Uh oh, that ain't supposed to be happening, row-size: %d not the expected: %d", row.length, size*size));
			}
		}
		puzzle = p;
	}
